...
class GUI_Manager:
    ...
    def _init_header_bar(self, master):
        ...
        def onSave():
            print("[onSave] self.sm.active = ", self.sm.active)
            self.sm.app[self.sm.active].file_frame.file_save()
      
  
        menubar = tk.Menu(self.window)

        filemenu = tk.Menu(menubar, tearoff=0)
        filemenu.add_command(label="Create new session", command=self._create_session)
        ...
        option.add_command(label="Default application", command=self._launch_default_application)
        option.add_command(label="Plot Q vs Power")
    ...
    def _create_session(self):
        self.sm.add_session("Untitled")
        #self._init_default_application(self.sm.sessions["Untitled"])

    #def _init_Plot_Q_vs_Power(self):

    def _launch_default_application(self):

        #self.sm.add_session("Untitled")
        '''session_tab = self.sm.sessions[self.sm.active]

        topFrame = Frame(session_tab)  # Added "container" Frame.
        topFrame.pack(side=TOP, fill=X, expand=1, anchor=N)
        Bottom = Frame(session_tab, bd=4, relief="ridge")
        Bottom.pack(side=BOTTOM, fill=X, expand=1, anchor=S)
        self._init_topFrame(topFrame)
        self._init_Bottom(Bottom)
        self._init_header_bar(tk.Frame(self.window, relief=tk.RAISED, borderwidth=2))'''

        self.sm.app[self.sm.active] = Default_Application_Manager(self.sm.get_active(), self)
        print("self.sm.active=",self.sm.active)
